...
yolov8l = YOLO("yolov8l.pt")  # load a pretrained model (recommended for training)
yolov8x = YOLO("yolov8x.pt")  # load a pretrained model (recommended for training)
...
yolo11l = YOLO("yolo11l.pt")  # load a pretrained model (recommended for training)
yolo11x = YOLO("yolo11x.pt")  # load a pretrained model (recommended for training)

# project.version(DATASET_VERSION).deploy(model_type=”yolov8”, model_path=f”{HOME}/runs/detect/train/”)
# Train the model with MPS
yolo11x.train(name = "yolo11x_test1",
              data = data_name,
